...
export default {
  ...
  mounted () {
    ...
    var headBobAnimation = this.$anime.timeline({
      easing: 'linear',
      loop: true,
      targets: [head, headphones]
    })
      .add({
        translateY: 5,
        rotateX: 10
    ...
    var note4Animation = this.$anime.timeline({ easing: 'linear', loop: true, targets: note4, duration: 750 }).add({ translateY: -25, opacity: 1 }, Math.random() * (+max - +min) + +min).add({ translateY: -50, opacity: 0 })
  }
}
</script>
